...
> Documentación completa en [docs/](./docs/): [ARCHITECTURE](./docs/ARCHITECTURE.md), [CONVENTIONS](./docs/CONVENTIONS.md), [RECIPES](./docs/RECIPES.md), [DESIGN](./docs/DESIGN.md).
>
> **Antes de generar o modificar UI** (JSX que consuma `@biaenergy/ui`), leé [`docs/DESIGN.md`](./docs/DESIGN.md) — es la fuente única de reglas del DS: qué componente usar, qué token semántico, qué jerarquía de botones, qué patrón de forms, qué evitar. Cada regla tiene un ID estable (`R1`…`R13`, `AP1`…) — citalos en comentarios inline o commits cuando tomes una decisión no obvia (ej. `// [R7] FancyButton para primary`).
...
Codex usa automáticamente estos skills cuando son relevantes:
...
Si necesitas aplicar estos patrones manualmente, consulta los archivos en [.Codex/skills/](./.Codex/skills/).
