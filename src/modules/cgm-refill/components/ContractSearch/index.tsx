...
import type { Contract, SearchContractField } from '../../models/cgm-refill.interface';
...
export const ContractSearch = ({ locale, onSelect }: ContractSearchProps) => {
  ...
  const handleSelect = (contract: Contract) => {
    onSelect(contract);
    setQuery(String(field === 'id' ? contract.id : field === 'sic' ? contract.sic : contract.name));
    setOpen(false);
  };
      ...
      <div className="flex gap-2">
        ...
        <div className="relative flex-1">
          ...
          {open && query.trim().length >= 1 && (
            <div className="bg-bg-white-0 border-stroke-soft-200 absolute z-10 mt-1 w-full rounded-lg border shadow-lg">
              {isFetching ? (
