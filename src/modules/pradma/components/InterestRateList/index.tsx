...
import { useSearchInterestRates } from '../../data';
            ...
            <Table.Root>
              <Table.Header>
                <Table.Row>
                  ...
                  <Table.Head className="text-right">{d.columns.interestPercentage}</Table.Head>
              ...
              <Table.Body>
                {data.data.map(ir => (
                  <Table.Row key={ir.id} {...clickableRowProps(() => openEdit(ir.id))}>
                    <Table.Cell className="text-text-soft-400 tabular-nums">{ir.id}</Table.Cell>
                    <Table.Cell className="text-text-strong-950 font-medium tabular-nums">
                      {ir.year}
                    </Table.Cell>
                    <Table.Cell className="tabular-nums">
                      {formatNumericDate(ir.startDate)}
                    </Table.Cell>
                    <Table.Cell className="tabular-nums">
                      {formatNumericDate(ir.endDate)}
                    </Table.Cell>
                    <Table.Cell className="text-right tabular-nums">
                      {formatPercent(ir.percentage)}
                    </Table.Cell>
                    <Table.Cell className="text-right tabular-nums">
                      {formatPercent(ir.interestPercentage)}
                    </Table.Cell>
