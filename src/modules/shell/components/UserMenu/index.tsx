...
export const UserMenu = ({ dict }: UserMenuProps) => {
  ...
  // Fallback al perfil persistido en localStorage cuando Firebase trae
  // photoURL vacío (puede pasar en re-logins con el token sin scope completo).
  const storedUser = useAuthStore(selectUser);
  const photoURL = user?.photoURL ?? storedUser?.photoURL ?? null;
    ...
    <div className="shrink-0 px-1.5 pt-2 pb-3">
      <Dropdown.Root>
        <Dropdown.Trigger asChild>
          ...
          >
            <UserAvatar photoURL={photoURL} initials={initials} alt={displayName} />
            <span
              className={cn(
