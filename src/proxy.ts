...
export const proxy = (request: NextRequest) => {
  ...
  // Solo validamos presencia del token — la firma la valida el backend en
  // cada request. Si la cookie está pero el token expiró, el backend
  // responde 401 y el http_client se encarga de limpiar sesión y redirigir.
  const session = request.cookies.get(SESSION_TOKEN_COOKIE)?.value;
